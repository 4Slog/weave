...
class ConnectionsPainter extends CustomPainter {
  ...
  ConnectionsPainter({required this.blocks});
  ...
  bool shouldRepaint(ConnectionsPainter oldDelegate) {
    ...
    if (blocks.length != oldDelegate.blocks.length) return true;
    
    // Check if any block position changed
    for (int i = 0; i < blocks.length; i++) {
      if (i >= oldDelegate.blocks.length) return true;
      if (blocks[i].position != oldDelegate.blocks[i].position) return true;
      
      // Check if connections changed
      for (int j = 0; j < blocks[i].connections.length; j++) {
        if (j >= oldDelegate.blocks[i].connections.length) return true;
        if (blocks[i].connections[j].connectedToId != 
            oldDelegate.blocks[i].connections[j].connectedToId) return true;
      }
    }
