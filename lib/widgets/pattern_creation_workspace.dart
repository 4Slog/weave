...
class PatternCreationWorkspace extends StatefulWidget {
  ...
  @override
  _PatternCreationWorkspaceState createState() => _PatternCreationWorkspaceState();
}
...
class _PatternCreationWorkspaceState extends State<PatternCreationWorkspace> with SingleTickerProviderStateMixin {
  ...
  void _updateHint() {
    // Get hint from mentor service
    final hintData = _mentorService.getContextualHint(
      blockCollection, 
      // For now we use default user progress, would be enhanced later to use actual user
      null, 
    );
  ...
  void _removeBlock(String blockId) {
    ...
    // Play sound if available
    widget.audioService?.playEffect(AudioType.cancelTap);
  ...
  /// Attempt to connect blocks that are close to each other
  void _attemptConnections(BlockModel block) {
    // Iterate through connections on this block
    for (var connection in block.connections) {
      // Skip if already connected
      if (connection.connectedToId != null) continue;
      
      // Calculate global position of this connection
      final connectionPos = Offset(
        block.position.dx + connection.position.dx,
        block.position.dy + connection.position.dy,
      );
      
      // Check against all other blocks' connections
      for (var otherBlock in blockCollection.blocks) {
        // Skip same block
        if (otherBlock.id == block.id) continue;
        
        for (var otherConnection in otherBlock.connections) {
          // Skip if already connected
          if (otherConnection.connectedToId != null) continue;
          
          // Calculate other connection position
          final otherConnectionPos = Offset(
            otherBlock.position.dx + otherConnection.position.dx,
            otherBlock.position.dy + otherConnection.position.dy,
          );
          
          // Check distance
          final distance = (connectionPos - otherConnectionPos).distance;
          if (distance < 15) { // Connection threshold
            // Check if connection types are compatible
            if (_blockService.canBlocksConnect(
                block, connection.id, 
                otherBlock, otherConnection.id)) {
              
              // Make the connection
              connection.connectedToId = otherBlock.id;
              connection.connectedToPointId = otherConnection.id;
              otherConnection.connectedToId = block.id;
              otherConnection.connectedToPointId = connection.id;
              ...
              return; // Only make one connection at a time
            }
          }
        }
      }
