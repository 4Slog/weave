...
class TTSSettings {
  ...
  /// Gets the appropriate voice based on current settings
  String getAppropriateVoice() {
    // Default voices based on gender
    final Map<VoiceGender, String> defaultVoices = {
      VoiceGender.male: 'en-us-x-sfg#male_1',
      VoiceGender.female: 'en-us-x-sfg#female_1',
      VoiceGender.neutral: 'en-us-x-sfg#neutral_1',
    };

    // If we have a specific voice set, use that
    if (voice.isNotEmpty) {
      return voice;
    }

    // Otherwise, use the default voice for the selected gender
    return defaultVoices[voiceGender] ?? 'en-us-x-sfg#female_1';
  }
  ...
  // Implementation moved to line 315
