...
class EngagementService {
  ...
  Future<void> _loadEngagementMetrics() async {
    try {
      ...
      // Get activity counts
      final activityJson = await _storageService.getSetting('activity_counts');
      if (activityJson != null && activityJson is Map) {
  ...
  void recordChallengeAttempt({
    ...
    if (success) {
      _learningService.completeChallenge(
        challengeId,
        difficulty: difficulty,
      );
      
      // Check for badges based on challenges
      _badgeService.checkForNewBadges(_learningService.userProgress?.userId ?? 'default');
    }
  }
  
  /// Record story progression
  void recordStoryProgress({
  ...
  Future<void> _saveEngagementMetrics() async {
    try {
      ...
      await _storageService.saveSetting('activity_counts', _activityCounts);
      
      // Save recent engagement events
      await _storageService.saveSetting('recent_engagement_events', _engagementEvents);
    } catch (e) {
